...
class Media {
  ...
  update(scroll, direction) {
    ...
    this.plane.scale.x = this.baseScale.x * this.currentScale;
    this.plane.scale.y = this.baseScale.y * this.currentScale;
  ...
  onResize({ screen, viewport } = {}) {
    ...
    this.baseScale = {
      x: (this.viewport.width * (490 * this.scale)) / this.screen.width,
      y: (this.viewport.height * (490 * this.scale)) / this.screen.height
    };
...
class App {
  ...
  checkHover(e) {
    ...
    this.medias.forEach(m => (m.targetScale = 1));
    let hoveredIndex = -1;
    if (hits && hits.length) {
      const hit = hits[0];
      const media = this.medias.find(m => m.plane === hit);
      if (media) {
        media.targetScale = 1.3;
        hoveredIndex = media.index;
      }
    }
...
export default function CircularGallery({
  ...
  return (
    <div className="circular-gallery" ref={containerRef}>
      <div className="circular-gallery-gradient-left"></div>
      <div className="circular-gallery-gradient-right"></div>
      <div
        style={{
          position: 'absolute',
          top: 'calc(50% + 170px)',
